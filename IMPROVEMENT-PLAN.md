...
- [x] **F7. Network segmentation.** Created `space-server_monitoring` bridge network (2026-05-24). node-exporter, cAdvisor, alertmanager-discord, Loki, and Promtail moved off `space-server_web` — app containers can no longer reach metrics (9100, 8080) or log API (3100) directly. Prometheus, Alertmanager, and Grafana dual-homed (web + monitoring). Further `edge`/`apps`/`mail` split deferred — would require changes across all app repos for minimal additional security gain given Traefik must join each network anyway. Single `web` network mixes edge, monitoring, app data. Split into `edge` (Traefik only public-facing), `apps`, `mail`, `monitoring`. Backend DBs (Ghost MySQL, portfolio MySQL/Redis) should never touch `edge`.
...
- [x] **F27. `.env` files world-readable (mode 0644) + password reuse across services.** `chmod 600` applied on polaris2. DB passwords rotated (portfolio-api + Ghost) in 2026-05-24 session — stored in `~/space-server/.credentials.md` (chmod 600). Structural secret management deferred to F16 (SOPS).
...
- [x] **F35. Ghost backed by EOL MySQL 5.7** (EOL Oct 2023). Migrated to `mysql:8.0` in `cativo23/ghost-blog` `compose.prod.yml` (2026-05-24). Added `--default-authentication-plugin=mysql_native_password` so Ghost's mysql2 driver works over internal Docker network without SSL. MySQL 8.0 upgrades the data dir in-place on first start. Deploy on polaris2: `git pull && docker compose -f compose.prod.yml up -d db` then verify logs, then `docker compose -f compose.prod.yml up -d ghost`.
...
- [x] **F36. `~/deploy/` zone has no git repo** — resolved by F3 (external stacks copied into `deploy/` in this repo).
- [x] **F39. Log persistence (Loki + Promtail)** — resolved by F6 (Loki stack with 7-day retention).
...
- [ ] **F46. Offsite "full-host-down" alert to Discord (dead-man switch).** Context: 2026-06-04 Grafana 504 outage went unnoticed because nothing alerted. Fixed the in-cluster gap that day by wiring Uptime-Kuma → Discord (native, free) across all 11 monitors — so app/edge failures (incl. 504s) now ping Discord. **Remaining gap:** Kuma and Alertmanager both run on polaris2, so a full host outage notifies nobody. Better Stack was trialed and dropped (free tier has no Discord; Slack is OAuth-only, no pasteable webhook for the Discord `/slack` trick). **Plan:** healthchecks.io free (Hobbyist $0 — Discord/Slack/webhook integrations are NOT plan-gated, only check-count and SMS/phone credits are). Create one check (ping ≤1 min, grace ~2 min) + Discord integration; add a 1-min cron on polaris2 that curls the ping URL. Host dies → pings stop → healthchecks (offsite) alerts Discord. Requires Carlos to create the healthchecks.io account (account creation is his to do); wiring can be done via browser afterward. Related: F44 (synthetic auth-regression probe).
